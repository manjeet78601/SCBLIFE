...
export class ScbHeaderComponent extends BaseApp implements OnInit {
  @Input("in") scbHeaderInputs: ScbHeaderInputs = new ScbHeaderInputs();
  ...
  navigatePage() {
    if (this.scbHeaderInputs.backNavTo) {
      ...
      let navData = this.scbHeaderInputs.backNavTo;
      console.log('called back', navData);
      this.scbHeaderInputs.nav.push(page, { "data": navData });
    } else {
